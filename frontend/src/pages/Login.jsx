 ...
 * Split-panel layout per design_guidelines.json (auth_split_hero).
 * Step 1: email + password → Step 2: TOTP if MFA required.
 */
...
const S = {
  ...
  left:  {
    flex:'0 0 45%', background:`linear-gradient(160deg,${navy} 0%,${teal} 100%)`,
    display:'flex', flexDirection:'column', justifyContent:'center',
    alignItems:'center', padding:48, color:'#fff',
  },
  ...
  input: {
    ...
    color:'#0F172A', background:'#fff', boxSizing:'border-box', marginBottom:16,
  },
  btn:   {
    width:'100%', padding:13, background:navy, color:'#fff', border:'none',
...
export default function Login() {
  ...
  const [stage,    setStage]    = useState('credentials'); // 'credentials' | 'mfa' | 'reset'
  ...
  const [resetStage,   setResetStage]   = useState('init'); // 'init' | 'confirm'
  ...
  return (
    <div style={S.page}>
      ...
      <div style={S.left}>
        ...
        <div style={{ fontSize:15, opacity:0.85, textAlign:'center', maxWidth:280, lineHeight:1.6 }}>
          HIPAA-compliant AI medical coding and billing — built for independent practices and billing companies.
        </div>
        <div style={{ marginTop:48, display:'flex', flexDirection:'column', gap:14, width:'100%', maxWidth:260 }}>
          {['ICD-10-CM / PCS coding','CPT & HCPCS assignment','MS-DRG grouper','NCCI edit validation','Denial risk scoring'].map(f=>(
            <div key={f} style={{ display:'flex', gap:10, alignItems:'center', fontSize:13, opacity:0.9 }}>
      ...
      <div style={S.right}>
        <div style={S.card}>
          ...
          {stage === 'credentials' && (
            <>
              <h2 style={{ fontSize:24, fontWeight:600, color:navy, marginBottom:4 }}>Sign in</h2>
              ...
              <form onSubmit={handleLogin}>
                ...
                <div style={{ textAlign:'right', marginTop:-12, marginBottom:16 }}>
                  <button type="button" onClick={()=>{setStage('reset');setError('');}}
          ...
          {stage === 'mfa' && (
            <>
              ...
              <form onSubmit={handleMFA}>
                ...
                <button type="submit" style={S.btn} disabled={loading||totp.length<6}
                  data-testid="mfa-submit-btn">
                  {loading ? 'Verifying…' : 'Verify code'}
                </button>
                <button type="button" onClick={()=>{setStage('credentials');setTotp('');}}
                  style={{ ...S.btn, background:'transparent', color:navy,
                           border:`1.5px solid ${navy}`, marginTop:8 }}
